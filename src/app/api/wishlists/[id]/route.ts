...
export async function GET(_request: Request, { params }: { params: { id: string } }) {
    try {
        ...
        const wishlist = await services.wishlistService.getWishlist(params.id);
        
        if (!wishlist || wishlist.userId !== user.id) {
            return NextResponse.json({ error: 'Not found' }, { status: 404 });
        }
        
        const items = await services.wishlistService.getItems(params.id);
...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
    try {
        ...
        const updated = await services.wishlistService.updateWishlist(user.id, params.id, name);
...
export async function DELETE(_request: Request, { params }: { params: { id: string } }) {
    try {
        const user = await requireSessionUser();
        const services = await getServerServices();
        await services.wishlistService.deleteWishlist(user.id, params.id);
